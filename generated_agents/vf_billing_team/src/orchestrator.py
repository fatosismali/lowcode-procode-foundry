"""
Team orchestrator for "vf-billing-team".
...
def _client(config: TeamConfig, model: "str | None") -> FoundryChatClient:
...
def _wire(pattern: str, participants: "list[Agent]", by_name: "dict[str, Agent]", orch: dict):
    ...
    if pattern == "handoff":
        ...
        builder = HandoffBuilder(
            name="vf_billing_team", participants=participants
...
async def run_team(task: str, config: "TeamConfig | None" = None) -> str:
    ...
    async with AsyncExitStack() as stack:
        ...
        if messages:
            return "\n".join(
                f"[{m.author_name or 'assistant'}] {m.text}" for m in messages
            )
        return str(final)
...
async def main() -> None:
    ...
    task = "{\"userQuery\": \"How much is my latest bill and has it been paid?\", \"selectedAccountReference\": null}\n"
    logger.info("Running 'vf-billing-team'...")
